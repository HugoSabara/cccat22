...
test("Não deve criar uma conta com e-mail inválido", function () {
    expect(() => Account.create("John Doe", "john.doe@gmail", "97456321558", "asdQWE123")).toThrow(new Error("Invalid email"));
});

test("Não deve criar uma conta com document inválido", function () {
    expect(() => Account.create("John Doe", "john.doe@gmail.com", "974563", "asdQWE123")).toThrow(new Error("Invalid document"));
});

test("Não deve criar uma conta com password inválido", function () {
...
test("Não deve fazer um saque se não tiver saldo", function () {
    const account = Account.create("John Doe", "john.doe@gmail.com", "97456321558", "asdQWE123");
    account.deposit("USD", 500);
    expect(() => account.withdraw("USD", 1000)).toThrow(new Error("Insufficient funds"));
});